    ...
    <v-container style="font-family: 'Kanit', sans-serif">
      ...
      <v-row>
        <v-col lg="6" sm="12" md="12">
          <v-card
            v-for="(item, index) in allItemInCart"
            :key="index"
            class="mx-auto my-12 font"
          ...
          >
            ...
            <div class="pr-3 pb-3" align="right">
              <v-icon @click="deleteItem(index, item)" large
        ...
        <v-col lg="6" sm="12" md="12">
          ...
          <br />
          <center>
            <v-btn title color="#D7ECFF" large @click="$router.push('/')">
              เลือกซื้อหนังสือต่อ
            </v-btn>
          </center>
        </v-col>
  ...
  methods: {
      ...
      if (confirm(text) == true) {
        try {
          const result = await axios.delete(
            `http://localhost:3000/deleteItem/${item.item_no}/${this.$store.state.user.cart.cart_id}`
          );
    ...
    async getItemInCart() {
      const result = await axios.get(
        `http://localhost:3000/itemsInCart/${this.$store.state.user.cart.cart_id}`
      );
