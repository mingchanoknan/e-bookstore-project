    ...
    <v-container style="font-family: 'Kanit', sans-serif">
      ...
      <v-row>
        <v-col lg="6" sm="12" md="12">
          ...
          <v-divider></v-divider><br />
          <p>
            <b>จำนวน {{ allItemInCart.length }} รายการ</b>
          </p>
          <p>
            <b>ราคารวมสินค้า {{ totalPrice }} THB</b>
          </p>
          <center>
            <v-btn
      ...
      if (confirm(text) == true) {
        try {
          const result = await axios.put(
            `http://localhost:3000/payment/${this.$store.state.user.cart.cart_id}/${this.$store.state.user.customer_id}`
          );
